...
void CRenderClip::GenerateRoundClip(HDC hDC, RECT rc, RECT rcItem, int width, int height, CRenderClip& clip)
{
    ...
    clip.hOldRgn = ::CreateRectRgnIndirect(&rcClip);
    clip.hRgn = ::CreateRectRgnIndirect(&rc);
    HRGN hRgnItem = ::CreateRectRgn(rcItem.left, rcItem.top, rcItem.right + 1, rcItem.bottom + 1);
...
BOOL CRenderEngine::TextOut(HDC hdc, int nXStart, int nYStart, LPCTSTR lpString, int cbString)
{
	RECT rcTemp;
	::DrawText(hdc, &lpString[0], _tcslen(lpString), &rcTemp, DT_CALCRECT);
	rcTemp.right = nXStart + (rcTemp.right - rcTemp.left);
	rcTemp.left = nXStart;
	rcTemp.bottom = nYStart + (rcTemp.bottom - rcTemp.top);
	rcTemp.top = nYStart;
	return ::DrawText(hdc, &lpString[0], _tcslen(lpString), &rcTemp, DT_LEFT | DT_VCENTER) != 0;
}
                                    ...
                                    BYTE uFade, bool hole, bool xtiled, bool ytiled)
{
    ...
    static LPALPHABLEND lpAlphaBlend = (LPALPHABLEND) ::GetProcAddress(::GetModuleHandle(_T("coredll.dll")), _T("AlphaBlend"));
...
void CRenderEngine::DrawGradient(HDC hDC, const RECT& rc, DWORD dwFirst, DWORD dwSecond, bool bVertical, int nSteps)
{
    ...
    static PGradientFill lpGradientFill = (PGradientFill) ::GetProcAddress(::GetModuleHandle(_T("coredll.dll")), _T("GradientFill"));
...
void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle)
{
    ...
    while( *pstrText != _T('\0') ) {
        ...
        else if( !bInRaw &&  *pstrText == _T('<') && pstrText[2] == _T('>') && (pstrText[1] == _T('{')  || pstrText[1] == _T('}')) )
        {
            ...
            if( bDraw && bLineDraw ) CRenderEngine::TextOut(hDC, pt.x, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, &pstrText[1], 1);
            pt.x += szSpace.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
        ...
        else if( !bInRaw &&  *pstrText == _T(' ') )
        {
            ...
            if( bDraw && bLineDraw ) CRenderEngine::TextOut(hDC, pt.x,  pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, _T(" "), 1);
            pt.x += szSpace.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
            pstrText++;
        }
        else
        {
            ...
            if( bDraw && bLineDraw ) {
				...
				CRenderEngine::TextOut(hDC, ptPos.x, ptPos.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, pstrText, cchSize);
                RETAILMSG(1, (TEXT(" DrawHtmlText %s\r\n"), pstrText)); 
				if( pt.x >= rc.right && (uStyle & DT_END_ELLIPSIS) != 0 ) 
                    CRenderEngine::TextOut(hDC, ptPos.x + szText.cx, ptPos.y, _T("..."), 3);
            }
            pt.x += szText.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
            pstrText += cchSize;
